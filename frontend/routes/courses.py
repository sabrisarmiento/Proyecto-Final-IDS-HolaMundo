...
def courses():
    ...
    try:
        res = requests.get('http://127.0.0.1:5000/courses', headers=headers)
        courses_data = res.json().get("courses", []) if res.status_code == 200 else []
        return render_template('courses.html', courses=courses_data)
    except Exception as e:
        print(f"Error en listado de cursos: {e}")
